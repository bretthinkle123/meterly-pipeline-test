...
| `test_events_rls_backstop.py` | Same adversarial table-owner proof as `test_usage_rollup_rls_backstop.py`, applied to `events`: the `events_tenant_isolation` policy (created in `0001`), once `FORCE`d by migration `0005`, confines a non-superuser table-owner connection to its own tenant's rows with the `api_key_id` filter removed (`test_rls_confines_table_owner_read_when_app_filter_is_absent`), and fails closed (zero rows) when `app.current_api_key_id` is never set (`test_rls_denies_all_rows_to_owner_when_tenant_setting_is_unset`). Fails against the pre-`0005` schema, passes after. |
